/**
 * The shapes this pack used to throw, kept only to be compared against the ones
 * that replaced them.
 *
 * Nothing reads this but the confetti playground, which draws it beside the
 * current set so the two can be judged side by side. It is scaffolding: once
 * the choice is made, this file, its alias in `apps/web/vite.config.ts` and the
 * playground's old/new toggle all go.
 *
 * Left exactly as it was, including the comments — a comparison against a copy
 * someone has tidied is not a comparison. Note what those comments claim, since
 * it is the thing that changed: a piece was one flat colour, and every piece of
 * detail had to be a hole punched in the silhouette.
 *
 * ---
 *
