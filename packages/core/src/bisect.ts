 ...
 *
 * The dictionary and the inflection tables are both one line per key, sorted:
 * `forms.txt.gz` (873,140 lines once the whole dictionary ships) and
 * `paradigms.txt.gz` (18,302 lines, but 10 MB of them). Building either into a
 * `Map` costs more than reading it did, for a feature used a handful of times a
 * session, so neither is: the file is held as one string and bisected where it
 * lies. That is what lets the same index serve a phone and the CLI unchanged.
 */
