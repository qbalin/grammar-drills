...
- Does the topic end on a heading for a page the student has not reached?
...
| 2026-08-24 | *not signed off — machine check only* | the 23 Bennett topics and 15 Lane topics whose text moved; §317-318 and §319-322 read side by side | The book's mixed-case centred captions were being filed under the section they were printed *between*, so 23 Bennett topics ended on the heading of the next page — §317-318 on "Conditional Sentences of the First Type.", which is §319's. They now open the section they head, and carry `⟦=⟧`, so they render as headings rather than as a stray sentence: 175 of Bennett's 192 read as prose before and 17 as list items, and none as a heading. Checked by reparsing both books and diffing: ids, refs, titles, families, order and `readingOnly` byte-identical across all 149 Bennett and 668 Lane topics, and both coverage manifests unchanged — only `text` moved, in 52 topics and 39. The three figures that moved are in BASELINE.json. **Still wanted from a human:** Lane's 72 paradigm captions (`regor, am ruled.`, `laudō, praise.`) were reclassified by the same change and now head their tables instead of standing above them as prose — seven of them as divider rows inside the table. That is the largest visual change here and no reader has seen it. |
